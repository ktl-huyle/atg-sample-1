...
public class searchExecutionFiles {
    
    private static def execute_functional_method(Map data) {
        
        "Step 1: Click on div sprint15Run1"
        
        WebUI.enhancedClick(findTestObject('AI-Generated/Page_team_project_test_runs/div_sprint15Run1'))
        ...
        WebUI.setText(findTestObject('AI-Generated/Page_team_project_executions_files/input_search'), data['input_search'])
    }
    
    private static def execute_with_data_source(String datasource, int rowIndex) {
        TestData testData = findTestData(datasource)
        Map data = [:]
        data['input_search'] = testData.getValue('input_search', rowIndex)
        execute_functional_method(data)
    }
    
    private static def execute_without_data_source() {
        Map data = [:]
        data['input_search'] = 'default_data'
        execute_functional_method(data)
    }
